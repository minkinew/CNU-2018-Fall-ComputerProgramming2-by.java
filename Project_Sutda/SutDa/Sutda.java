...
{	
	...
	//1카드(5명의 카드위치)  //왼쪽위1,왼쪽아래2, 유저3, 오른쪽아래4, 오른쪽위5
	int locate_x[] = {40, 40, 380, 715, 715};	
	int locate_y[] = {10, 220, 460, 220, 10};
	
	//2카드   //왼쪽위1,왼쪽아래2, 유저3, 오른쪽아래4, 오른쪽위5
	...
	JButton bt2 = new JButton("체크");
	JButton bt3 = new JButton("콜");
	JButton bt4 = new JButton("삥");
	JButton bt5 = new JButton("따당");
	JButton bt6 = new JButton("맥스");
	int exit=0;
	...
	public void paint(Graphics g) {
		...
		//방장
		g.drawString("방장 : " + (sun+1) +"p", 10, 460);
		g.drawString("==========================", 10, 480);
		g.drawString("* 방장만 체크와 삥을 할수 있음.", 10, 500);
		g.drawString("* 다이를 누른후 잠시 기다려 주세요.", 10, 520);
		g.drawString("* 1~2배팅은 매너 맥스배팅", 10, 540);
		
		
		
		//카드 뒷그림 불러오기.
		hide_img = Toolkit.getDefaultToolkit().getImage("중립화투.png");
		//5명에게 줄 카드 10장 불러오기.(섯다는 20장중 10장만 사용함.)
		for(int i=0; i<10; i++){
		...
		if(key==1)money_change1(454);
			...
			if(re_bet==0){g.drawString( "맥스", 341, 90); }//+2, +20	//맥스글자
			else{g.drawString(""+s1, 341, 90);}
			
			g.setColor(Color.pink);
			g.fillRect(339, 280, 30, 30);
			g.setColor(Color.BLACK);
			if(re_bet==0){g.drawString( "맥스", 341, 300); }//+2, +20	//맥스글자
			else{g.drawString(""+s2, 341, 300);}
			
			g.setColor(Color.pink);	
			g.fillRect(490, 405, 30, 30);
			g.setColor(Color.BLACK);
			g.drawString("체크", 492, 425);
			
			
			g.setColor(Color.pink);
			g.fillRect(630, 280, 30, 30);
			g.setColor(Color.BLACK);
			if(re_bet==0){g.drawString( "맥스", 632, 300); }//+2, +20	//맥스글자
			else{g.drawString(""+s4, 632, 300);}
			
			g.setColor(Color.pink);
			g.fillRect(630, 70, 30, 30);
			g.setColor(Color.BLACK);
			if(re_bet==0){g.drawString( "맥스", 632, 90); }//+2, +20	//맥스글자
			...
			g.fillRect(490, 405, 30, 30);
			g.setColor(Color.BLACK);
			g.drawString( "맥스", 492, 425);//+2, +20	//맥스글자
		...
		if(state==9){	
			...
			if(key==1)money_change1(454);
				...
				//패의 숫자를 넣었을때 각패에따라 확률을 지정하여 배팅을 시도.
		...
		if(state==10){	//모든카드를 보여준다.
			...
			if(s1!="다이") g.drawString("" +class_name[0], 140, 200);
			else{ player[0] =0;}	//"다이"라면 계산에서 제외된다.0이 젤낮다.
			if(s2!="다이") g.drawString("" +class_name[1], 140, 410);
			else{ player[1] =0;}	//"다이"라면
			//3g.drawString("" +class_name[1], 140, 410);
			if(s4!="다이") g.drawString("" +class_name[3], 800, 410);
			else{player[3] =0; }	//"다이"라면
			if(s5!="다이") g.drawString("" +class_name[4], 800, 200 );
			else{ player[4] =0;}	//"다이"라면
					...
					if(player[1]==player[2]){	
						if(player[2] == player[3]){
							if(player[3] == player[4]){
								g.drawString("**" +class_name[1] +"**", 400, 60);
								g.drawString(""+(player_order[1]+1)+ "p와"+ (player_order[2]+1)+"p와"+ (player_order[3]+1)+"p와"+ (player_order[4]+1)+"p만 재경기", 400, 100);
								//System.out.println(player_order[1]+ player_order[2]+player_order[3]+player_order[4] +"만 재경기");
							}else{
								g.drawString("**" +class_name[1] +"**", 400, 60);
								g.drawString(""+(player_order[1]+1)+ "p와"+ (player_order[2]+1)+"p와"+ (player_order[3]+1)+"p만 재경기", 400, 100);
								//System.out.println(player_order[1] +"와"+ player_order[2]+player_order[3]+"만 재경기");
							}
						}else{
							g.drawString("**" +class_name[1] +"**", 400, 60);
							g.drawString(""+(player_order[1]+1)+ "p와"+ (player_order[2]+1)+"p만 재경기", 400, 100);
				...
				if(player[0]==player[1]){	
					if(player[1] == player[2]){
						if(player[2] == player[3]){
							if(player[3] == player[4]){
								g.drawString("**" +class_name[0] +"**", 400, 60);
								g.drawString(""+(player_order[0]+1)+ "p와"+ (player_order[1]+1)+"p와"+ (player_order[2]+1)+"p와"+ (player_order[3]+1)+(player_order[4]+1)+"p만 재경기", 400, 100);
								//System.out.println(player_order[0]+ player_order[1]+player_order[2]+player_order[3]+player_order[4] +"만 재경기");
							}else{
								g.drawString("**" +class_name[0] +"**", 400, 60);
								g.drawString(""+(player_order[0]+1)+ "p와"+ (player_order[1]+1)+"p와"+ (player_order[2]+1)+"p와"+ (player_order[3]+1)+"p만 재경기", 400, 100);
								//System.out.println(player_order[0]+ player_order[1]+player_order[2]+player_order[3] +"만 재경기");
							}							
						}else{
							g.drawString("**" +class_name[0] +"**", 400, 60);
							g.drawString(""+(player_order[0]+1)+ "p와"+ (player_order[1]+1)+"p와"+ (player_order[2]+1)+"p만 재경기", 400, 100);
							//System.out.println(player_order[0] +"와"+ player_order[1]+player_order[2]+"만 재경기");
						}
					}else{
						g.drawString("**" +class_name[0] +"**", 400, 60);
						g.drawString(""+(player_order[0]+1) +"p와"+ (player_order[1]+1)+"p만 재경기", 400, 100);
	...
	public void actionPerformed(ActionEvent ae){
		...
		if(sun == 2){		//사용자가 선일때
			...
			}else if(ae.getSource()==bt2){	//체크
				if(end!=10){
					key=2;
					if(state==0)state=6;	//맥스가 눌러질때마다 state6으로
					if(po==1)state=9;		//모든카드를 받으면 po1이된다.
											//state9에서 마지막배팅후, state10으로..
					repaint();
				}
				
			}else if(ae.getSource()==bt3){	//콜
				if(end!=10){
					key=3;
					if(state==0)state=6;	//맥스가 눌러질때마다 state6으로
					if(po==1)state=9;		//모든카드를 받으면 po1이된다.
	...
	public void money_change2(){	//체크
		check();
	}
	
	public void money_change3(){	//콜
		call();
		pan_money = pan_money + bet_money;	//+bet
	}
	public void money_change4(){	//삥
		bbing();
		bet_money += 20;			//20원추가
		pan_money = pan_money + bet_money;	
	}
	public void money_change5(){	//따당
		dadang();
		bet_money += (bet_money/2);	// 배팅액 bet/2배
		pan_money = pan_money + bet_money;	
	}
	
	public void money_change6(){	//맥스
		max();
		bet_money *= 2;	// 배팅액 2배
		pan_money = pan_money + bet_money;	
	}
	...
	public void money_change2(int user){	//체크
		check();
	}
	public void money_change3(int user){	//콜
		call();
		u_money -= bet_money;	//자금 = 자금 - 배팅액
		pan_money = pan_money + bet_money;	//+bet
		
	}
	public void money_change4(int user){	//삥
		bbing();
		bet_money += 20;			//20원추가
		pan_money = pan_money + bet_money;	
	}
	public void money_change5(int user){	//따당
		dadang();
	...
	public void money_change6(int user){	//맥스
		max();
	...
	public int what_class(int num1, int num2){
		...
		if(result <85){
			...
			if(num3 >= 10)num3 = num3 - 10; //10을 빼준다.
